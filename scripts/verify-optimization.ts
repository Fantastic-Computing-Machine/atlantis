...
async function main() {
  ...
  if (tagCheck1?.usageCount !== 1)
    throw new Error(`Tag usage should be 1, got ${tagCheck1?.usageCount}`);
  ...
  if (tagCheck2?.usageCount !== 1)
    throw new Error(`Tag usage should be 1, got ${tagCheck2?.usageCount}`);
  ...
  if (tagCheck3?.usageCount !== 2)
    throw new Error(`Tag usage should be 2, got ${tagCheck3?.usageCount}`);
  ...
  if (tagCheck4?.usageCount !== 1)
    throw new Error(`Tag usage should be 1, got ${tagCheck4?.usageCount}`);
  ...
  if (tagCheck5?.usageCount !== 0)
    throw new Error(`Tag usage should be 0, got ${tagCheck5?.usageCount}`);
