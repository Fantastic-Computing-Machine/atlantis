...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    const isValid =
      ...
      crypto.timingSafeEqual(Buffer.from(confirmationCode.toUpperCase()), Buffer.from(stored.code));

    if (!isValid) {
      return NextResponse.json({ error: 'Confirmation code does not match' }, { status: 400 });
    }
