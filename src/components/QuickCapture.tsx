...
export function QuickCapture() {
  ...
  if (!isOpen) {
    return (
      <Button variant="outline" size="sm" className="gap-2 text-xs" onClick={() => setIsOpen(true)}>
        <Plus className="h-3.5 w-3.5" />
        Quick capture
        <kbd className="bg-muted text-muted-foreground hidden items-center gap-0.5 rounded border px-1.5 py-0.5 text-[10px] sm:inline-flex">
          Alt + N
        </kbd>
      </Button>
    );
    ...
    <div className="text-muted-foreground flex flex-wrap items-center gap-3 text-[10px]">
      {shortcuts.map((s) => (
        <span key={s.key} className="inline-flex items-center gap-1">
          <kbd className="bg-muted rounded border px-1 py-0.5 font-mono">{s.key}</kbd>
          <span>{s.label}</span>
        </span>
      ))}
    </div>
