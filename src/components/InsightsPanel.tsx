import { FileCode2, Hash, PenSquare, Star, StickyNote, Clock, Sparkles, AlertCircle, Brain, Plus } from 'lucide-react';
import type { DashboardStats, FiletypeStats, TopTag, TodoItem, ActivityItem, StaleItem, KnowledgeStats } from '@/lib/dashboard-data';
...
export function InsightsPanel({ topTags, filetypes, todos, activity, staleContent, rediscovery, knowledgeStats }: InsightsPanelProps) {
    return (
                    ...
                    <ul className="space-y-2">
                        {todos.map((todo, i) => (
                            <li key={i} className="flex items-start gap-2 text-xs">
                                <span className="inline-block h-4 w-4 border rounded mt-0.5 shrink-0" />
                                <div className="min-w-0 flex-1">
                                    <p className="truncate">{todo.text}</p>
                                    <Link href={`/notes/${todo.noteId}`} className="text-muted-foreground hover:text-primary truncate block">
                                        {todo.noteTitle}
                                    </Link>
                                </div>
                            </li>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {topTags.map((tag) => (
                            <Link
                                key={tag.id}
                                href={`/tags/${tag.slug}`}
                                className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium border transition-colors hover:border-primary/50"
                    ...
                    <div className="flex flex-wrap gap-2">
                        {filetypes.map((ft) => (
                            <span key={ft.language} className="inline-flex items-center gap-1 text-[10px]">
                                <code className="bg-muted px-1.5 py-0.5 rounded">{ft.language}</code>
                        ...
                        {staleContent.map((item) => (
                            <li key={`${item.type}-${item.id}`} className="text-xs">
                                ...
                                >
                                    <span>{item.emoji}</span>
                                    <span className="truncate flex-1">{item.title}</span>
                                    <span className="text-amber-500 shrink-0">{item.daysSinceUpdate}d ago</span>
                                </Link>
                            </li>
                        ))}
                    </ul>
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
            {items.map((item, idx) => (
                <span key={item.label} className="inline-flex items-center gap-1.5">
                    ...
                    {item.href ? (
                        <Link href={item.href} className="font-semibold hover:underline">{item.value}</Link>
                    ) : (
