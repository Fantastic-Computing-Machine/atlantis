...
export function TodoList({ value, onChange }: TodoListProps) {
  ...
  const handleToggle = (id: string, checked: boolean) => {
    const newItems = items.map((item) => (item.id === id ? { ...item, completed: checked } : item));
    setItems(newItems);
    serialize(newItems);
  };
...
function TodoItem({
  ...
  return (
    <Reorder.Item
      ...
      className="bg-card group hover:border-muted-foreground/20 flex items-start gap-3 rounded-lg border p-3 transition-colors"
    >
      <div
        className="text-muted-foreground hover:text-foreground mt-1 shrink-0 cursor-grab touch-none p-1"
      ...
      <Textarea
        ...
        className={cn(
          'min-h-[2rem] flex-1 resize-none border-none bg-transparent px-0 py-1 shadow-none focus-visible:ring-0',
      ...
      <Button
        variant="ghost"
        size="icon"
        className="text-muted-foreground hover:text-destructive mt-1 h-8 w-8 shrink-0 opacity-0 transition-opacity group-hover:opacity-100"
